document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!productListString) {
    document.getElementById("moda-container").innerHTML =
  ...
  const petProducts = productList.filter(
    (p) => p.productCategory === "Moda"
  );

  const container = document.getElementById("moda-container");

  if (!petProducts.length) {
    container.innerHTML =
      "<p>Moda kategorisinde ürün bulunamadı.</p>";
    return;
  }
