document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!productListString) {
    document.getElementById("ayakkabı-canta-container").innerHTML =
  ...
  const petProducts = productList.filter(
    (p) => p.productCategory === "Ayakkabı&Çanta"
  );

  const container = document.getElementById("ayakkabı-canta-container");

  if (!petProducts.length) {
    container.innerHTML =
      "<p>Ayakkabı&Çanta kategorisinde ürün bulunamadı.</p>";
    return;
  }
