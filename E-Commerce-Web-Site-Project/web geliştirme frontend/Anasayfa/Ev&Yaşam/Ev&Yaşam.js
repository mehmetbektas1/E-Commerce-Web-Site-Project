document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!productListString) {
    document.getElementById("ev-yasam-container").innerHTML =
  ...
  const petProducts = productList.filter(
    (p) => p.productCategory === "Ev & Yaşam"
  );

  const container = document.getElementById("ev-yasam-container");

  if (!petProducts.length) {
    container.innerHTML =
      "<p>Ev Yaşam kategorisinde ürün bulunamadı.</p>";
    return;
  }
